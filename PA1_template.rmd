...
```{r}
stepsforeachday<-aggregate(activity$steps, by=list(Day=activity$date), FUN=sum)
print(stepsforeachday)
hist(stepsforeachday$x, xlab="Steps", main="Total Steps per Day")

```

### Calculation Mean and Median
```{r}
Avgsteps <- as.integer(mean(stepsforeachday$x,na.rm=T))
paste("The average number of steps taken each day:", Avgsteps)
Quartile2<- median(stepsforeachday$x,na.rm=T)
...
```{r}
Modifieddatasteps<-aggregate(Modifieddata$steps, by=list(Date=Modifieddata$date), FUN=sum)
print(Modifieddatasteps)
hist(Modifieddatasteps$x, col="Red", xlab="Steps" , main="#Steps taken per day")
hist(stepsforeachday$x, col="blue", xlab="Steps", main="#Steps taken per day", add=T)
legend("topright", c("NA Data", "Non-NA Data"), fill=c("Red", "blue") )
```

### Mean of Imputed Missing Values
``` {r}
MeanData<-as.integer(mean(Modifieddatasteps$x))
paste("The average number of steps taken each day:", MeanData)
```

### Median of Imputed Missing Values
``` {r}
Mediandata<- median(Modifieddatasteps$x)
paste("The Median number of steps taken each day:", Mediandata)
```
